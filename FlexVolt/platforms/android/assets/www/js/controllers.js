...
//        };
    ...
    .controller('SettingsCtrl', ['$scope', '$state', '$timeout', '$ionicModal', '$ionicPopover', '$http', 'flexvolt', 'appLogic',
    function($scope, $state, $timeout, $ionicModal, $ionicPopover, $http, flexvolt, appLogic) {
        var currentUrl = $state.current.url;
        console.log('currentUrl = '+currentUrl);
        ...
        $scope.submitBugReport = function(){
            ...
            .catch(function (err){
                console.log('ERROR: Failed connection to bugreport server');
                $scope.bugmodal.hide();
            }); } catch (err){
              console.log('ERROR: Caught error trying to send bugreport');
              $scope.bugmodal.hide();
            }
